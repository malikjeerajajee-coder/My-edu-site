...
const resources = [
  ...
  { href: '/boards',          icon: 'graduation-cap',label: 'All Boards',      count: BOARDS.length },
];
---
      ...
      <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {BOARDS.map(b => (
          <a href={url(`/board/${b.slug}`)} class="row group">
            <span class="tile">
              <Icon name="graduation-cap" size={18} strokeWidth={2.2} />
            </span>
  ...
  <section class="section">
    <div class="section-inner">
      ...
      <div class="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
        {resources.map(r => (
          <a href={url(r.href)} class="group rounded-lg border border-slate-200 bg-white p-4 transition-colors hover:border-[#1d4ed8]">
            <div class="flex items-center justify-between gap-2">
              <span class="tile">
                <Icon name={r.icon} size={18} strokeWidth={2.2} />
              </span>
              <span class="text-base font-extrabold tracking-tight text-slate-900">{r.count}</span>
            </div>
            <div class="mt-3 text-[13px] font-extrabold tracking-tight text-slate-900">{r.label}</div>
          </a>
        ))}
      </div>
    </div>
