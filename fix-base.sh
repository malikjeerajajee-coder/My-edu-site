...
# 1. Create a URL helper that prepends the base path
...
# 2. Rewrite BaseLayout to use the helper for every internal link
...
<body class="min-h-screen bg-white">
  ...
  <aside class="fixed inset-y-0 left-0 z-40 hidden w-[260px] flex-col border-r border-neutral-200 bg-white lg:flex">
    ...
    <div class="border-t border-neutral-200 p-3">
      <form action={url('/search')} method="get" role="search" class="flex items-center gap-2 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2.5 transition-colors focus-within:border-[#0620ed] focus-within:bg-white">
  ...
  <header class="sticky top-0 z-30 flex h-16 w-full items-center gap-3 border-b border-neutral-200 bg-white/95 px-4 backdrop-blur-xl lg:hidden">
    ...
    <a href={url('/search')} class="ml-auto grid h-10 w-10 place-items-center rounded-lg border border-neutral-200 bg-white text-neutral-700 transition-colors active:bg-neutral-100" aria-label="Search">
        ...
        <div class="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <ul class="mt-4 space-y-2.5 text-sm text-neutral-500">
              <li><a href={url('/classes')} class="hover:text-[#0620ed]">Classes</a></li>
              <li><a href={url('/subjects')} class="hover:text-[#0620ed]">Subjects</a></li>
          ...
          <div>
            ...
            <ul class="mt-4 space-y-2.5 text-sm text-neutral-500">
              <li><a href={url('/books')} class="hover:text-[#0620ed]">Textbooks</a></li>
              <li><a href={url('/gazettes')} class="hover:text-[#0620ed]">Gazettes</a></li>
              <li><a href={url('/search')} class="hover:text-[#0620ed]">Search</a></li>
            </ul>
          </div>
        </div>
  ...
  <style is:global>
    main { animation: pageFadeIn 0.28s ease-out both; }
  ...
  <script is:inline>
    (function () {
      ...
      if (openBtn) openBtn.addEventListener('click', function (e) { e.preventDefault(); e.stopPropagation(); open(); });
...
echo ""
echo "════════════════════════════════════════════"
echo "  Base path fix applied"
echo "════════════════════════════════════════════"
echo ""
echo "  Created: src/lib/url.ts"
echo "    - url('/notes')   → /My-edu-site/notes"
echo "    - url('/')        → /My-edu-site/"
echo ""
echo "  Updated: src/layouts/BaseLayout.astro"
echo "    - All internal links now use url()"
echo "    - Sidebar, drawer, footer, brand mark, search"
echo ""
echo "  NEXT STEPS:"
echo "    1. Run:  npm run build"
echo "    2. Push:  git add . && git commit -m 'Fix base path' && git push"
echo "    3. Wait ~90s for GitHub Actions"