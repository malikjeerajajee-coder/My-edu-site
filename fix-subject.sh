...
// BISE picker: only for BISE-aware provinces on BISE-specific resource types
const showBISEPicker = hasBISEs(String(boardSlug)) && isType && (subjectSlug === 'past-papers' || subjectSlug === 'gazettes');
const bises = getBISEsForProvince(String(boardSlug));
---
{board && typeData ? (
<BaseLayout
  title={`${board.name} Class ${cls} ${typeData.label} | Parhayi`}
  description={showBISEPicker
    ? `All ${typeData.label.toLowerCase()} for ${board.full} Class ${cls}. Choose your specific BISE to see papers for your board.`
    : `Free ${typeData.label.toLowerCase()} for ${board.full} Class ${cls}. Downloads organized by subject and year.`}
>
    ...
    <div class="mx-auto max-w-[1200px] px-5 py-12 sm:px-7 lg:px-10 lg:py-14">
      <div class="mb-6">
        ...
        <p class="mt-2 text-sm text-slate-600 max-w-2xl">
          {board.name} has {bises.length} separate BISEs, and each sets its own {typeData.label.toLowerCase()} for Class {cls}. Pick your board to see the right ones.
        </p>
      </div>

      <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
        {bises.map(bise => (
          <a href={url(`/board/${boardSlug}/${bise.slug}/class-${cls}/${subjectSlug}`)} class="row group">
            <span class="tile">
...
<BaseLayout
  title={`${subjectData.board.name} Class ${cls} ${subjectData.subject} | Parhayi`}
