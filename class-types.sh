...
echo "════════════════════════════════════════════"
echo "  Creating /class/[class]/[type] pages"
echo "════════════════════════════════════════════"
echo ""

git branch -f backup-pre-class-types 2>/dev/null || true
echo "  ✓ backup-pre-class-types created"
echo ""

mkdir -p 'src/pages/class/[class]'

# ═════════════════════════════════════════════════════════
#  /class/[class]/[type].astro — one file handles all 7 types
# ═════════════════════════════════════════════════════════
...
const TYPES = {
  ...
  'guess-papers': {
    label: 'Guess Papers',
    singular: 'paper',
    collection: 'guessPapers',
    detailBase: '/guess-papers',
    icon: 'sparkles',
  ...
  'pairing-schemes': {
    ...
    singular: 'scheme',
    collection: 'pairingSchemes',
    detailBase: '/pairing-schemes',
    icon: 'list',
...
export async function getStaticPaths() {
  const results: any[] = [];
  const classesWithType: Set<string> = new Set();
  ...
  const map: Record<string, any[]> = {
    notes, quizzes, books, gazettes, pastPapers: papers, guessPapers: guess, pairingSchemes: pairing,
  };

  for (const typeSlug of Object.keys(TYPES)) {
    const cfg = (TYPES as any)[typeSlug];
    const coll = map[cfg.collection] || [];
    const classes = new Set<string>();
    coll.forEach((i: any) => classes.add(String(i.data.class)));
    for (const cls of classes) {
      results.push({ params: { class: cls, type: typeSlug } });
    }
  }
  return results;
}
...
ASTRO
echo "  ✓ /class/[class]/[type].astro created"

# ═════════════════════════════════════════════════════════
#  Add line-clamp utility if missing
# ═════════════════════════════════════════════════════════
python3 <<'PY'
import pathlib
p = pathlib.Path('src/styles/global.css')
s = p.read_text()
if '.line-clamp-2' not in s:
    s = s.rstrip() + '''
...
'''
    p.write_text(s)
    print('  ✓ line-clamp-2 utility added')
else:
    print('  · line-clamp-2 already present')
PY
...
echo ""
echo "  Test these (from Class 9 hub → click each resource):"
echo "    /class/9/books/                → Class 9 books, grouped by subject"
echo "    /class/9/notes/                → Class 9 notes"
echo "    /class/9/quizzes/              → Class 9 quizzes"
echo "    /class/9/past-papers/          → Class 9 papers"
echo "    /class/9/gazettes/             → Class 9 gazettes"
echo "    /class/9/guess-papers/         → Class 9 guess papers"
echo "    /class/9/pairing-schemes/      → Class 9 schemes"
echo ""
echo "  Then click any item → should open detail page:"
echo "    /textbook/[id]        (books)"
echo "    /note/[id]            (notes)"
echo "    /quiz/[id]            (quizzes)"
echo "    /past-papers/[id]     (papers)"
echo "    /guess-papers/[id]    (guess)"
echo "    /pairing-schemes/[id] (schemes)"
echo "    /gazettes/[id]        (gazettes)"
echo ""
echo "  Push when happy:"
echo "    git add . && git commit -m 'Add class type pages' && git push"
echo ""
echo "  Revert: git checkout backup-pre-class-types"
echo "════════════════════════════════════════════════════════"