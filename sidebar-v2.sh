#!/bin/bash
# sidebar-v2.sh — Save My Exams-style tree sidebar
set -e
cd ~/my-edu-site

echo "▶ Backing up BaseLayout.astro"
cp src/layouts/BaseLayout.astro src/layouts/BaseLayout.astro.bak

echo "▶ Rewriting sidebar"
python3 << 'PYEOF'
import re
...
# ─── 1. Frontmatter nav data ───
NEW_NAV_DATA = '''  // ─── Nav tree (Save My Exams style) ───
  ...
  const navTree: any[] = [
    ...
    },
    {
      ...
      items: [
        ...
        href: `/board/${b.slug}`,
        label: b.name,
        children: CLASS_NUMS.map(n => ({
          href: `/board/${b.slug}/class-${n}`,
          label: `Class ${n}`,
        })),
      })),
    },
  ...
  const branchActive = (item: any) =>
    isActive(item.href) || (item.children || []).some((c: any) => isActive(c.href));
'''
...
# ─── 2. Shared nav markup (used twice) ───
NAV_MARKUP = '''<nav class="flex-1 overflow-y-auto px-2 py-3">
        {navTree.map(sec => (
          <div class="mb-5">
...
# ─── 3. Desktop sidebar (full replacement) ───
...
# ─── 4. Mobile drawer (full replacement) ───
...
if npm run build; then
  ...
  echo "Push:     git add . && git commit -m 'Sidebar v2 — Save My Exams-style tree nav' && git push"
  echo ""
  echo "To roll back:"
  echo "  cp src/layouts/BaseLayout.astro.bak src/layouts/BaseLayout.astro"
else
  echo ""
  echo "════════════════════════════════════════"
  echo "  ✗ BUILD FAILED — restoring backup"
  echo "════════════════════════════════════════"
  cp src/layouts/BaseLayout.astro.bak src/layouts/BaseLayout.astro
  exit 1
fi