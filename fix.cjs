...
// 1. Create Sidebar.astro
const sidebarDir = path.join('src', 'components');
...
fs.writeFileSync(path.join(sidebarDir, 'Sidebar.astro'), sidebarCode);
...
// Remove old sidebars and insert <Sidebar />
if (layout.includes('<!-- ═══ DESKTOP SIDEBAR ═══ -->')) {
  layout = layout.replace(
    /<!-- ═══ DESKTOP SIDEBAR ═══ -->[\s\S]*?<!-- ═══ CONTENT ═══ -->/,
    '<Sidebar />\n  <!-- ═══ CONTENT ═══ -->'
  );
...
// Update padding
if (layout.includes('<div class="lg:pl-[250px]">')) {
  layout = layout.replace(/<div class="lg:pl-\[250px\]">/, '<div class="lg:pl-[272px]">');
  console.log('✅ Updated main content padding');
}
