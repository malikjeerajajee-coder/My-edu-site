...
# 1. Check content config
echo "1. Content config file:"
if [ -f "src/content.config.ts" ]; then
  echo "   EXISTS: src/content.config.ts"
else
  echo "   MISSING: src/content.config.ts — recreating..."
fi
echo ""

# 2. Check content folders
echo "2. Content files:"
...
if [ ! -f "src/content/notes/math-10-ch1.md" ]; then
  echo "   Missing notes — regenerating sample content..."
  MISSING=1
fi
...
EOF
  ...
  mkdir -p public/pdfs
  for f in math-10-ch1 physics-9-ch1 chemistry-10-ch1 biology-11-ch1 english-9-essays physics-9-textbook math-10-textbook chemistry-10-textbook biology-11-textbook bise-lahore-10-2025 bise-karachi-9-2024 bise-rawalpindi-10-2024; do
    touch "public/pdfs/$f.pdf"
  done

  echo "   Regenerated all sample content."
fi
...
<body style="font-family: monospace; padding: 20px;">
  ...
  <h2>First note</h2>
  <pre>{JSON.stringify(notes[0]?.data ?? null, null, 2)}</pre>
</body>
</html>
EOF
...
echo ""
echo "The /debug page shows how many items each collection has."
echo "If those numbers are >0 but the home page still shows 0,"
echo "tell me and I'll dig deeper."